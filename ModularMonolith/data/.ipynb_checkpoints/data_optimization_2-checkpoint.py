...
def _load_ffill_limits_from_raw2() -> dict[str, int | None]:
    """Load per-file ffill_limit from raw_data_2 manifest.

    Returns a mapping of dataset basename -> ffill_limit (may be None for unbounded).
    """
    if not RAW2_MANIFEST_PATH.exists():
        raise FileNotFoundError(f"Run data_optimization_1.py first to build {RAW2_MANIFEST_PATH}")
    ...
    if not isinstance(files, list):
        raise ValueError(f"Invalid manifest files list: {RAW2_MANIFEST_PATH}")
...
def _build_monthly_growth_features(
    ...
    # Resample to month-end to ensure consecutive months.
    # Note: this may introduce a small additional lag vs release dates, but keeps
    # month-over-month deltas well-defined even when raw2 has gaps.
    ...
    # Stationary transforms by series semantics:
    # - CPI/INDPRO are index-like levels: use log-diffs (approx growth rates).
    # - UNRATE is a rate (percentage): use absolute differences (percentage points).
    s_m = pd.to_numeric(df_monthly[col], errors="coerce")
    if basename in {"CPIAUCSL", "INDPRO"}:
        log_s = pd.Series(np.nan, index=s_m.index, dtype="float64")
        valid = s_m > 0
        log_s[valid] = np.log(s_m[valid])
        df_monthly[f"{col}_mom"] = log_s.diff(1)
        df_monthly[f"{col}_yoy"] = log_s.diff(12)
    elif basename == "UNRATE":
        df_monthly[f"{col}_mom"] = s_m.diff(1)
        df_monthly[f"{col}_yoy"] = s_m.diff(12)
    else:
        # Generic fallback: pct_change for level-like macro series.
        df_monthly[f"{col}_mom"] = s_m.pct_change(1)
        df_monthly[f"{col}_yoy"] = s_m.pct_change(12)
